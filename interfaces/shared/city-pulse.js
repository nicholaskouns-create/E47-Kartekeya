...
function emitQuery(p){
 const q=p?.query_packet;
 if(!q||q.schema!=="CITY-QUERY/1.0")return;
 window.CITY_QUERY_STATE=q;
 document.dispatchEvent(new CustomEvent("city:query",{detail:q}));
}
export function installCityPulse({surface="CITY",onPulse=null}={}){
 let bc=null,last=null;
 const receive=p=>{if(!valid(p))return;last=p;window.CITY_PULSE_STATE=p;document.dispatchEvent(new CustomEvent("city:pulse",{detail:p}));emitQuery(p);onPulse?.(p)};
