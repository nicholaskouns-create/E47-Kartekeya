(() => {
  ...
  // Source filter survives the Atlas reframe without dominating the front door.
  ...
  function boundAt(step) {
    ...
    const denom = P0 + C0 * r;
    const weightFloor = P0 / denom;
    const leakCeil = (C0 * r) / denom;
    return {weightFloor, leakCeil};
  }
  ...
  function drawKernel() {
    ...
    ctx.strokeStyle=css('--mint') || '#b9e6c8';
    ctx.lineWidth=2*dpr;
    ctx.beginPath();
  ...
  function drawLeak() {
    ...
    ctx.strokeStyle=css('--cyan') || '#63d7cf';
    ctx.lineWidth=2*dpr;
    ctx.beginPath();
    for(let s=0;s<=TMAX;s++){
      const b=boundAt(s);
