...
function extendTerrainFleet(){const w=window.CITY_SKYRMION_TERRAIN3D;if(!w?.ready||w.fleet?.length!==6)return false;const jacob=w.fleet[5],sky=jacob.clone(true);sky.name='SKYRMION_RUNTIME2';sky.userData={...jacob.userData,cameraDistance:39,cameraHeight:12,cameraLookAhead:66,runtime2Vehicle:'skyrmion'};sky.scale.multiplyScalar(.92);sky.traverse?.(o=>{if(o.material){o.material=o.material.clone?.()||o.material;o.material.color?.setHex?.(0x6ee7d8);o.material.emissive?.setHex?.(0x082a31);}});sky.visible=false;w.craftRoot.add(sky);w.fleet.splice(5,0,sky);w.setActiveCraft?.(rt.vehicleIndex);return true;}
